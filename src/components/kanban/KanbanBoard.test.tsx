...
const defaultWorkspaceState = {
    workspaces: [{ id: "ws-1", name: "Default Workspace", created_at: "2026-01-01" }],
...
describe("Kanban Feature Integration Suite", () => {
    ...
    it("Scenario 2: should create a new task via AddNewTask input and add it to BACKLOG", async () => {
        ...
        const quickAddSpy = vi.spyOn(kanbanServices, "quickAddTask").mockResolvedValueOnce({
            status: "success",
            data: {
                task: {
                    id: "task-new",
        ...
        await waitFor(() => {
            expect(quickAddSpy).toHaveBeenCalledWith("ws-1", { title: "Newly Created Task" });
    ...
    it("Scenario 3: should update task position optimistically and rollback if server move fails", async () => {
        ...
        expect(store.getState().kanban.tasks.find((t) => t.id === "task-1")?.columnId).toBe("TO_DO");
        ...
        store.dispatch(
            moveTaskThunk.rejected(
                null,
                "requestId-123",
                { taskId: "task-1", status: "TO_DO", order: 1, previousTasksSnapshot: mockTasks.map(t => ({ ...t, desc: t.description, columnId: t.status })) },
                { message: "Server connection error", previousTasksSnapshot: mockTasks.map(t => ({ ...t, desc: t.description, columnId: t.status })) }
            )
        );

        // Position rolled back to BACKLOG
        expect(store.getState().kanban.tasks.find((t) => t.id === "task-1")?.columnId).toBe("BACKLOG");
    });
    ...
    it("Scenario 5: should open GlobalConfirmDeleteModal when deleting task, and handle confirmation & localStorage suppression", async () => {
        ...
        const deleteSpy = vi.spyOn(kanbanServices, "deleteTask").mockResolvedValueOnce({
            status: "success",
    ...
    it("Scenario 6: should re-fetch tasks when activeWorkspaceId changes", async () => {
        const getTasksSpy = vi.spyOn(kanbanServices, "getTasksByWorkspace").mockResolvedValue({
            status: "success",
            data: { tasks: mockTasks },
        });
