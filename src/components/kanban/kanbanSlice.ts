  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<IBackendTask[]>) => {
          state.isLoading = false;
      ...
      .addCase(fetchTasksThunk.rejected, (state, action) => {
        state.isLoading = false;
        state.error =
          (action.payload as string | undefined) ?? "Failed to fetch tasks";
      })
      // Quick Add Task
      ...
      .addCase(quickAddTaskThunk.rejected, (state, action) => {
        state.isLoading = false;
        const errorMsg =
          (action.payload as string | undefined) ?? "Failed to create task";
      ...
      .addCase(moveTaskThunk.rejected, (state, action) => {
        const payload = action.payload as
          { message: string; previousTasksSnapshot: Task[] } | undefined;
        if (payload?.previousTasksSnapshot) {
          state.tasks = payload.previousTasksSnapshot;
        }
        const errorMsg =
          payload?.message ?? "Failed to move task. Position restored.";
        state.error = errorMsg;
        toast.error(errorMsg);
      })
      // Update Task Detail
      ...
      .addCase(
        updateTaskDetailThunk.fulfilled,
        (state, action: PayloadAction<IBackendTask>) => {
          state.isLoading = false;
      ...
      .addCase(updateTaskDetailThunk.rejected, (state, action) => {
        state.isLoading = false;
        const errorMsg =
          (action.payload as string | undefined) ?? "Failed to update task";
        state.error = errorMsg;
        toast.error(errorMsg);
      })
      // Delete Task
      ...
      .addCase(deleteTaskThunk.rejected, (state, action) => {
        ...
        state.deletingTask = null;
        const errorMsg =
          (action.payload as string | undefined) ?? "Failed to delete task";
        state.error = errorMsg;
        toast.error(errorMsg);
      });
  },
