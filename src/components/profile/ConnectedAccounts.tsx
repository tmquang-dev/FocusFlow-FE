...
export function ConnectedAccounts() {
    ...
    const socialLinks = useAppSelector((state) => state.profile.user?.social_links);
    ...
    const handleUnlink = async (provider: "github" | "google") => {
        try {
            ...
            toast.success(`Đã hủy liên kết tài khoản ${provider === "github" ? "GitHub" : "Google"} thành công.`);
        } catch (err: unknown) {
    ...
    const handleLink = (provider: "github" | "google") => {
        toast.info(`Vui lòng đăng nhập qua ${provider === "github" ? "GitHub" : "Google"} để hoàn tất liên kết.`);
    };
