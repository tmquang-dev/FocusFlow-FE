...
export const authServices = {
  ...
  githubLogin: (payload: IGithubLoginPayload): Promise<IApiLoginSuccess> =>
    axiosClient.post("/v1/auth/github", payload),
  ...
  completeRegister: (
    payload: ICompleteRegisterPayload,
    token: string,
  ): Promise<IApiCompleteRegisterSuccess> =>
  ...
  ): Promise<IApiForgotPasswordSuccess> =>
    axiosClient.post("/v1/auth/password/forgot", payload),
  resendPasswordOtp: (
    payload: IResendPasswordOtpPayload,
  ): Promise<IApiResendPasswordOtpSuccess> =>
    axiosClient.post("/v1/auth/password/resend-otp", payload),
  verifyPasswordOtp: (
    payload: IVerifyPasswordOtpPayload,
  ): Promise<IApiVerifyPasswordOtpSuccess> =>
    axiosClient.post("/v1/auth/password/verify-otp", payload),
  resetPassword: (
    payload: IResetPasswordPayload,
    token: string,
  ): Promise<IApiResetPasswordSuccess> =>
